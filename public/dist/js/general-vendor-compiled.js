...
class FormDataJson {
  ...
  static toJson(el, options) {
    ...
    function isValidInput(input) {
      ...
      const inputType = (input.type || 'text').toLowerCase(); // ignore button values
    ...
    function recursion(inputs, values) {
      for (let key in inputs) {
        ...
        const objectKey = options.flatList ? row.name : key; // next level
  ...
  static reset(el) {
    ...
    function recursion(inputs) {
      for (let key in inputs) {
        const row = inputs[key]; // next level
  ...
  static setInputValue(row, newValue, triggerChangeEvent) {
    ...
    const input = row.input;
    const inputType = row.inputType; // ignore file input, they cannot be set
  ...
  static getFieldTree(el, isValidInput) {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      for (let j = 0; j < keyPartsLength; j++) {
        ...
        let newCurrentName = currentName ? currentName + '[' + keyPart + ']' : keyPart; // auto increment key part
